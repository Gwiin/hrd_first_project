...
README에는 예전 기준으로 서보가 1개처럼 보일 수 있지만, **실제 최신 코드 기준으로는 창문용 서보가 2개**입니다.  
그리고 두 서보는 mirrored 구조로 움직입니다.
...
최신 코드 기준 특징:
- 듀얼 서보 사용
- 두 서보를 약간 시간차를 두고(stagger) 움직임
- 창문이 casement 구조처럼 열리고 닫히는 동작을 고려한 설계
...
- 최신 구현에서는 조명 제어가 단순 LED가 아니라 WS2812 스트립 기반으로 확장되었고, 창문도 듀얼 서보 기반으로 동작한다.
